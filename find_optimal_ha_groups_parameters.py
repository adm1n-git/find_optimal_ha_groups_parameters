    ...
    for lb1_active_member_count in range(number_of_members,-1,-1):
        for lb2_active_member_count in range(number_of_members,-1,-1):
            if lb1_active_member_count >= sufficient_member_count:
                ...
                if "active" in lb1:
                    lb1_score = active_bonus + (weight*(lb1_active_member_count/sufficient_member_count));
                else:
                    lb1_score = weight*(lb1_active_member_count/sufficient_member_count);            
            if lb2_active_member_count >= sufficient_member_count:
                if "active" in lb2:
                    lb2_score = (active_bonus - 10) + weight;
                else:
                    lb2_score = weight;
            else:
                if "active" in lb2:
                    lb2_score = (active_bonus - 10) + (weight*(lb2_active_member_count/sufficient_member_count));
                else:
                    lb2_score = weight*(lb2_active_member_count/sufficient_member_count);
...
def find_optimal_ha_groups_parameters():
    ...
    for weight in range(0, 1000, 10):
        ...
        for active_member_count in range(number_of_members,-1,-1):
            ...
            print(f"\nLB2:\nActive Bonus: {active_bonus-10}, Number of Members: {number_of_members}, Minimum Member Count: {minimum_member_count}, Sufficient Member Count: {sufficient_member_count}, Weight: {weight}");
